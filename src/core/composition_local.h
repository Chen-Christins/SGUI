...
public:
    ...
    void pushAny(CompositionLocalKey* key, std::any value) {
        stacks_[key].push(std::move(value));
    }
...
public:
    ProviderModifier(CompositionLocalKey* key, std::any value,
...
public:
    explicit CompositionLocal(T defaultValue)
        : defaultValue_(std::move(defaultValue)) {
    }

    T current() const {
        return CompositionLocalScope::instance().get<T>(
            const_cast<CompositionLocal*>(this), defaultValue_);
    }

    std::shared_ptr<Widget> provides(T value, std::shared_ptr<Widget> child) const {
        return std::make_shared<ProviderModifier>(
            const_cast<CompositionLocal*>(this),
